...
try {
	...
	assert.match(bundle.text, /^## Policy file: \.pi\/verifier\/a-policy\.md/, "policy bundle starts with visible file boundary for first policy");
	...
	assert.match(__openspecVerifierTest.contextPacketPrompt(packet), /# Repository verifier policies/, "prompt includes policy section");
